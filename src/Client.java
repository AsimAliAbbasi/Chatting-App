...
public class Client implements ActionListener {
    ...
    Client() {
        ...
        ImageIcon i1 = new ImageIcon(new File("img.JPEG").getAbsolutePath());
        ...
        JLabel name = new JLabel("Asim Ali");
    ...
    // Playfair Decryption
    private static String playfairDecrypt(String ciphertext) {
        // Convert the key to uppercase and remove duplicate letters
        String key = "MAHNOORZAFAR";
        key = key.toUpperCase().replaceAll("[^A-Z]", "");
    ...
    private static void removePaddingX(StringBuilder text) {
    ...
    try (Socket s = new Socket("127.0.0.1", 6001)) {
        ...
        while (true) {
            ...
            // Separate the "Encrypted: " label from the actual encrypted message
            int labelIndex = msg.indexOf("Encrypted: ");
            String encryptedMsg = (labelIndex != -1) ? msg.substring(labelIndex + 11) : msg;

            // Decrypt the received message using Playfair
            String decryptedMsg = playfairDecrypt(encryptedMsg);

            // Display the decrypted message on the receiving side
            JPanel panel = formatLabel("Decrypted: " + decryptedMsg);
